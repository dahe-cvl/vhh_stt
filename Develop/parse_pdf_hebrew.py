...
from parse_config import is_int, is_timestamp, get_config

def is_comment(text):
    return (text[0] == '[' and text[-1] == ']') or (text[0] == ']' and text[-1] == '[')

def compose_condition(text):
...
class ParseDocument():
    ...
    def parse_lines(self, line_iterator):
        # build up line until number of '[' and ']' matches
        composed_text = ''
        for page_number, raw_line in line_iterator:
            text = ''.join(raw_line.itertext())

            # [ and { are sometimes mixed in this pdf
            text = text.replace('{', '[')
            text = text.replace('}', ']')
            # remove initials
            ...
            if page_number == 34 and composed_text[-13:] == 'Anm d. Tanskr':
                text = '[' # flip wrong bracket. otherwise parsing breaks

            text = composed_text + text

            # timestamps and comments
            if compose_condition(text):
                composed_text = text
                continue
            else:
                composed_text = ''
            ...
            # line is text
                ...
                # reverse character order if text contains hebrew
...
current_file = 'OH_ZP1_291'
file_name, cfg = get_config(current_file)
base_path = os.path.join(directory, os.path.splitext(file_name)[0])
...
tree = ET.parse(base_path + '.xml', parser=parser)
root = tree.getroot()
...
text = text.translate((str.maketrans('', '', punctuation_symbols)))
...
html_diff = get_parse_diff(base_path + '.pdf', plain_text)
